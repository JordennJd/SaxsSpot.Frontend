...
import {
    plotScatteringChart,
    plotScatteringChartAverage,
    plotScatteringChartAveragePng,
...
export const ScatteringCalculationChartPage = () => {
    ...
    const scatteringIdsQueryRaw = searchParams.get('scatteringIds');
    const scatteringIdsQuery = scatteringIdsQueryRaw
        ? scatteringIdsQueryRaw.split(',').map((s) => s.trim()).filter(Boolean)
        : [];

    const hasScatteringIdsQuery = scatteringIdsQueryRaw != null;
    ...
    const filename = `saxs_scattering_${isAverage ? 'avg' : 'single'}${seriesId ? `_${safePart(seriesId)}` : ''}${
        qFrom && qTo ? `_q_${safePart(qFrom)}-${safePart(qTo)}` : ''
    }.png`;

    const request = useMemo<PlotScatteringChartRequest>(() => {
    ...
    }, [scatteringIdsQueryRaw, scatteringIdsQuery.join(','), hasScatteringIdsQuery, id, isAverage, location.state]);

    const [chart, setChart] = useState<string>('');
    const [isLoading, setIsLoading] = useState(false);

    useEffect(() => {
        if (request.ScatteringCalculationIds.length === 0) return;
            ...
            try {
                if (isDownloadMode) {
                    const base64 = isAverage
                        ? await plotScatteringChartAveragePng(request)
                        : await plotScatteringChartPng(request);
                ...
                const result = isAverage
                    ? await plotScatteringChartAverage(request)
                    : await plotScatteringChart(request);
                const fullHtml = `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  <div id="chart-container">${result}</div>
                ...
                console.error('Error fetching SAXS chart:', error);
            } finally {
                setIsLoading(false);
            }
        };
